    ...
    *,
    ...
    try:
        ...
        if failures:
            ...
            # ⚠️ Причина обязана ехать вместе с фактом отказа. До 22.09.2026 здесь
            # оставалось только число порций: журнал прогонов честно говорил «снимок
            # неполон» и не говорил ни слова о том, почему, — а разбирать это потом
            # приходится по логу контейнера, который живёт до перезапуска.
            result.error = (
                f"порций не снялось: {result.chunks_failed}. Снимок неполон, и выдавать "
                f"его за полный нельзя. Причины: {'; '.join(failures[:3])}"
            )

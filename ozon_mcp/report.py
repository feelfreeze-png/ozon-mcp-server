...
class Report:
    ...
    def anomaly_counts(self) -> dict[str, int | None]:
        """Сколько строк в каждой аномалии — готовым числом, а не длиной списка.

        🔴 Заведено после живого прогона 21.09.2026: отчёт отдал 111 товаров с
        расходом и без заказов, а в сводку ушло **113**. Инструмент был прав и
        устойчив (два прогона подряд, совпадение с сырым рядом до строки) — ошибся
        тот, кто считал список глазами. Промпт требовал «ничего не считай сам», но
        длину списка всё равно приходилось считать: готового числа в ответе не было.
        Правило «числа приходят из инструмента» держится тем, что число в ответе
        действительно есть.

        `None` вместо нуля там, где проверка не выполнялась: «ноль аномалий» и
        «аномалию не проверяли» — разные утверждения, и ноль из них ложный.
        """
        counts: dict[str, int | None] = {}
        for title, rows in self.anomalies.items():
            if title.endswith("НЕ ПРОВЕРЕНО"):
                continue
            counts[title] = len(rows)
        if "заказы без остатка — НЕ ПРОВЕРЕНО" in self.anomalies:
            counts["заказы без остатка"] = None
        return counts
    ...
    def as_dict(self) -> dict[str, Any]:
        return {
            ...
            **{title: round(self.excluded.get(kind, 0.0), 2)
            ...
            # Счётчики идут ПЕРЕД списками: тот, кто пишет сводку, должен взять
            # готовое число, а не считать строки. См. `anomaly_counts`.
            "аномалий, штук": self.anomaly_counts(),

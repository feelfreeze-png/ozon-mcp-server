...
- не назван вовсе             → **аргумент не трогаем**, спрашивает диспетчер.
...
🔴 Последний пункт — не мелочь и не лень. Первая редакция подставляла здесь первый
магазин списка, «по умолчанию». Это возвращало дефект 23.09 через другую дверь:
модель, забывшая `shop_id` на одном из четырёх кабинетов, получала числа первого —
и писала их под заголовком того, о котором спрашивала. «Первый» вдобавок ничем не
обоснован: это порядок записи в переменной окружения, а не выбор владельца.

Подставлять здесь нечего, поэтому и не подставляем: вызов доходит до ветки
`_call_tool_impl`, которая на пустой `shop_id` уже умеет ответить «укажите магазин»
и перечислить доступные. Инструменты, которым `shop_id` не нужен (`ozon_list_shops`,
`ozon_degradations`), до этой ветки просто не доходят — и по этой же причине
проверку нельзя было поставить в `enforce`: он один на все 150 инструментов и
про схему вызываемого не знает.
...
class ForeignShopError(Exception):
    ...
    def __init__(self, requested: str, allowed: tuple[str, ...]) -> None:
        ...
        # 🔴 Список доступного НЕ попадает в текст исключения — только в атрибут.
        # `str(exc)` уходит в общую таблицу вызовов (`stats.record_call`), а она
        # не разделена по арендаторам: `ozon_degradations` отдаёт оттуда тексты
        # ошибок любому клиенту. Перечень магазинов отказавшего токена в этом
        # тексте означал бы, что достаточно разок попросить чужой магазин, чтобы
        # опубликовать соседям состав своего кабинета. В конверт клиенту список
        # кладёт `failures.classify` — тот уходит только спросившему.
        super().__init__(f"Магазин {requested!r} не доступен этому токену.")
...
def is_configured() -> bool:
    """Переменная задана — независимо от того, разобралась ли хоть одна запись.

    🔴 Отдельно от `is_enabled()`, и это не педантизм. Мусорное значение
    (`tok:||`, `:shop`, `токен_без_двоеточия`) отбрасывается поштучно; если
    отброшены ВСЕ записи, словарь пуст — и без этой функции сервер решил бы,
    что режим просто выключен. Дальше `_resolve_mcp_auth` при пустом
    `MCP_AUTH_TOKEN` пускает вообще без токена: опечатка в одной переменной
    открывала бы сервер настежь, и снаружи это выглядело бы как «всё работает».

    Отказ должен быть громким: переменная задана — значит, вход только по
    разобранному токену, а не «раз ничего не разобралось, пускаем всех».
    """
    return bool((os.environ.get("MCP_CLIENT_TOKENS") or "").strip())


def is_enabled() -> bool:
    """Задан ли хоть один РАЗОБРАННЫЙ клиентский токен."""
...
def enforce(arguments: dict) -> dict:
    ...
    if requested is None or requested == "":
        # Магазин не назван. Подставлять нечего — спросит диспетчер (см. докстринг).
        return arguments

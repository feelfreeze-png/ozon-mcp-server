...
def get_shop_keys(data_dir: Path, shop_id: str) -> dict[str, str]:
    ...
    🔴 Текст отказа НЕ перечисляет магазины установки. Прежняя редакция дописывала
    сюда `Доступные: [...]` — полный список идентификаторов всех арендаторов, — и
    этот текст уходил клиенту конвертом. То есть отменял фильтр `ozon_list_shops`,
    который прячет соседей намеренно («знание чужих идентификаторов — готовая
    половина атаки»): достаточно было спросить несуществующий магазин.

    Свои магазины клиент и так знает: их отдаёт `ozon_list_shops`, а при нескольких
    — ещё и подсказка диспетчера «Укажите shop_id», которая перечисляет ровно их.
    """
    shops = load_shops(data_dir)
    if shop_id not in shops:
        raise ValueError(f"Магазин '{shop_id}' не найден. "
                         f"Список своих магазинов даёт ozon_list_shops.")

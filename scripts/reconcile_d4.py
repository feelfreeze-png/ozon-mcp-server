...
🔴 **Имена складов сравниваются без учёта регистра И разделителя** — см. `_fold`.
Оба раза отказ моста выглядел как отсутствие данных, и оба раза чинилось это после
того, как числа были увидены: сперва регистр, затем дефис против подчёркивания.
Свёртка делается в Python: `lower()` SQLite кириллицу не сворачивает и дала бы тот же
ложный результат.

⚠️ **Ядро приёмки — «а + в + г».** Условие «б» (`product_sku.source='fbo'`) снято
решением владельца 22.09.2026: на этом кабинете оно невыполнимо по построению. Число
по дословному «б» остаётся в выдаче как `core_literal_b` — замером, а не критерием.
"""
...
#: Разделители, которыми два источника пишут одно и то же имя склада по-разному.
_SEPARATORS = str.maketrans({"-": "_", " ": "_"})


def _fold(name: str | None) -> str:
    """Свернуть имя склада к виду, в котором два источника сравнимы.

    🔴 **Сворачивается не только регистр, но и разделитель.** Замерено 22.09.2026:

        отчёт:  Ростов_на_Дону_РФЦ     снимок:  РОСТОВ-НА-ДОНУ_РФЦ
        отчёт:  Санкт_Петербург_РФЦ    снимок:  САНКТ-ПЕТЕРБУРГ_РФЦ

    Подчёркивание против дефиса. На свёртке одного регистра эти два склада выглядели
    **отсутствующими в снимке**, а их SKU уезжали в корзину «не проверено» — то есть
    отказ моста второй раз подряд выглядел как отсутствие данных. Первый раз это был
    регистр (`Казань_РФЦ_НОВЫЙ` против `КАЗАНЬ_РФЦ_НОВЫЙ`), и его чинили отдельно.

    Проверено, что свёртка безопасна: ни в одном из двух источников она **не склеивает**
    два разных склада (44 имени отчёта и 47 снимка — ноль склеек), а сопоставление
    выросло с 42 из 44 до **44 из 44**. Проверку держит
    `tests/test_acceptance_d4_threshold.py`.
    """
    return (name or "").casefold().translate(_SEPARATORS)
...
def reconcile(db_path: str, day: str, shop_id: str = "main") -> dict:
    ...
    return {
        ...
        # Ядро приёмки. Условие «б» снято решением владельца 22.09.2026 —
        # `docs/ACCEPTANCE-D4.md`. `core_literal_b` остаётся замером, а не критерием.
        "core": len(core_relaxed),
        "core_literal_b": len(core),
        "matched": len(matched),
...
def main() -> int:
    ...
    if args.json:
        ...
        return 0 if out["core"] >= MIN_CORE else 2
    ...
    print()
    print(f"ЯДРО приёмки (а + в + г): {out['core']}")
    print(f"  для справки — по снятому условию «б» дословно: {out['core_literal_b']}")
    ...
    if out["core"] < MIN_CORE:
        print(f"ИТОГ: НЕ ПРОВЕДЕНА — ядро {out['core']} SKU < {MIN_CORE}.")
